...
-(void)swipeableviewcellMoveFromSuperView:(LZSwipeableViewCell *)cell withDirection:(LZSwipeableViewCellSwipeDirection)direction;
@end
...
-(void)moveWith:(LZSwipeableViewCellSwipeDirection)direction{
    if ([self.LZPrivateDelegate respondsToSelector:@selector(swipeableviewcellMoveFromSuperView:withDirection:)]) {
        [self.LZPrivateDelegate swipeableviewcellMoveFromSuperView:self withDirection:direction];
    }
}
...
- (void)pan:(UIPanGestureRecognizer *)gestureRecognizer{
    ...
    //NSLog(@"x:%f----y:%f",xFromCenter,yFromCenter);
    if (yFromCenter<0) {
        if (xFromCenter>0) {
            if (yFromCenter*-1>xFromCenter) {
                //向上
                [self moveWith:LZSwipeableViewCellSwipeDirectionTop];
            }else if (yFromCenter*-1<xFromCenter){
                //向右
                [self moveWith:LZSwipeableViewCellSwipeDirectionRight];
            }
        }else if (xFromCenter<0){
            if (xFromCenter<yFromCenter) {
                //向左
                [self moveWith:LZSwipeableViewCellSwipeDirectionLeft];
            }else if (xFromCenter>yFromCenter){
                //向上
                [self moveWith:LZSwipeableViewCellSwipeDirectionTop];
            }
        }
...
- (void)afterSwipeAction{
    if (xFromCenter > ACTION_MARGIN) { // 右边飞走
        ...
        [self moveWith:LZSwipeableViewCellSwipeDirectionCenter];
...
-(void)swipeableviewcellMoveFromSuperView:(LZSwipeableViewCell *)cell withDirection:(LZSwipeableViewCellSwipeDirection)direction{
    [self.delegate swipeableView:self cardMoveDirectionAtIndex:cell.tag withDirection:direction];
}
